...
def test_build_emits_all_crawlers_for_profile1():
...
def test_db_run_start_fans_out_to_all_crawlers():
    ...
    assert "ATS Watchlist: Fetch Jobs" in target_names
...
def test_each_crawler_connects_to_merge_sources_with_unique_index():
    ...
    assert sorted(indexes) == [0, 1, 2, 3, 4, 5]
...
def test_build_emits_ats_watchlist_crawler_when_selected():
    tmp_dest = REPO / "profiles" / "profile_ci_ats_watchlist_80b2"
    tmp_dest.mkdir(exist_ok=True)
    try:
        (tmp_dest / "search.json").write_text(
            json.dumps({"crawlers": ["ats_watchlist"]}),
            encoding="utf-8",
        )

        wf = run_build(tmp_dest.name)

        node = next(n for n in wf["nodes"] if n["name"] == "ATS Watchlist: Fetch Jobs")
        assert node["id"] == "ats-watchlist-fetch"
        assert node["parameters"]["jsCode"].startswith("// ATS Watchlist:")

        merge = next(n for n in wf["nodes"] if n["name"] == "Merge Sources")
        assert merge["parameters"]["numberInputs"] == 1
        assert wf["connections"]["DB: Run Start"]["main"][0] == [
            {"node": "ATS Watchlist: Fetch Jobs", "type": "main", "index": 0}
        ]
        assert wf["connections"]["ATS Watchlist: Fetch Jobs"]["main"][0] == [
