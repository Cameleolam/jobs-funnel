// Arbeitnow remote feed: same API, remote-only filter, distinct search config.
const fs = require('fs');
...
const MAX_PAGES = config.an_remote_max_pages || 5;
const DELAY_MS = config.an_remote_delay_ms || 3000;
...
const titleKw = search.an_remote_title_keywords || [];
const tagKw = search.an_remote_tag_keywords || [];
const negKw = search.an_remote_negative_keywords || [];
...
for (let page = 1; page <= MAX_PAGES; page++) {
  ...
  try {
    body = await fetchWithRetry.call(this, {
      method: 'GET',
      url: `https://www.arbeitnow.com/api/job-board-api?remote=true&page=${page}`,
      json: true,
    });
    totalRequests++;
  } catch (e) {
  ...
  for (const job of data) {
    // The ?remote=true param is advisory; filter client-side too.
    ...
    if (!negMatch && (titleMatch || tagMatch) && created > cutoff) {
...
if (allJobs.length === 0 && errors.length > 0) {
  throw new Error(`AN-remote: all ${errors.length} page requests failed after retries. First error: ${errors[0].error}`);
}
